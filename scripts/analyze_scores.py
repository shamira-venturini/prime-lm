# scripts/analyze_scores.py (FINAL, COMPLETE VERSION)
# This FAST script reads the raw data and calculates s-PE and w-PE for BOTH Active and Passive targets.
    ...
    for corpus_dir in corpora_dirs:
        ...
        for i in tqdm(range(num_items), desc="Calculating scores"):
            ...
            try:
                ...
                data_x_px = torch.load(os.path.join(corpus_dir, 'x_px', f'item_{i}.pt'))  # Active Target | Active Prime
                data_x_py = torch.load(
                    os.path.join(corpus_dir, 'x_py', f'item_{i}.pt'))  # Active Target | Passive Prime
                ...
                if args.calculate_spe:
                    result['s_pe_active'] = wpe_x_px.sum() - wpe_x_py.sum()

                if args.calculate_wpe:
                    min_len_active = min(len(wpe_x_px), len(wpe_x_py))
                    result['w_pe_active'] = wpe_x_px[:min_len_active] - wpe_x_py[:min_len_active]
                    result['target_tokens_active'] = data_x_px['tokens'][
                        data_x_px['target_start_idx']:data_x_px['target_start_idx'] + min_len_active]

                # --- PASSIVE TARGET ANALYSIS (NEW SECTION) ---
                data_y_py = torch.load(
                    os.path.join(corpus_dir, 'y_py', f'item_{i}.pt'))  # Passive Target | Passive Prime
                data_y_px = torch.load(
                    os.path.join(corpus_dir, 'y_px', f'item_{i}.pt'))  # Passive Target | Active Prime

                wpe_y_py = calculate_log_probs(data_y_py['activations']['layer_-1_hx'], data_y_py['token_ids'],
                                               data_y_py['target_start_idx'])
                wpe_y_px = calculate_log_probs(data_y_px['activations']['layer_-1_hx'], data_y_px['token_ids'],
                                               data_y_px['target_start_idx'])

                if args.calculate_spe:
                    result['s_pe_passive'] = wpe_y_py.sum() - wpe_y_px.sum()

                if args.calculate_wpe:
                    min_len_passive = min(len(wpe_y_py), len(wpe_y_px))
                    result['w_pe_passive'] = wpe_y_py[:min_len_passive] - wpe_y_px[:min_len_passive]
                    result['target_tokens_passive'] = data_y_py['tokens'][
                        data_y_py['target_start_idx']:data_y_py['target_start_idx'] + min_len_passive]

                all_results.append(result)

            except FileNotFoundError:
                # This handles cases where one of the four files might be missing for an item
                continue
